...
- 指令执行结果出现异常而引起的中断
    - 程序性中断
- 中断的次序，优先级
    - 次序：中断响应次序和中断处理次序，响应次序由中断响应硬件中的排队器所决定，由高到低固定；实际处理中，中断的处理由中断处理程序完成，中断处理程序可被中断，这样处理次序与响应次序可以不同，通常只有比当前处理的中断请求高级的中断请求才能中断其处理，响应和处理完后再继续处理原请求；
	- 程序状态字(psw)中包含有中断级屏蔽位字段，假设系统有4个中断级，相应地每一级中断处理程序的现行PSW中都有4位中断级屏蔽位，屏蔽位为1表示对该级中断开放，允许其进入中断响应排队器，为0则对该级中断屏蔽，0，1规定在不同机器上定义不同；
- 总线的控制方式
    - 有分布式和集中式，这里只讲集中式
    - 集中式串行链接方式
    - 集中式定时查询方式
...
- 计算机系统执行通道程序完成输入输出工作时，通道程序的执行者
    - 通道处理机
- 通道流量的计算
    - 假设数据传送期间选择一次设备的时间ts，传送一个字节时间为td，则通道极限流量的计算为：
    - 字节多路通道，每次只选一台设备送一个字节，所以通道极限流量为1/(ts+td);
    - 数组多路通道每次选一台设备传K个字节，若要传N个字节，要分N/K次传送，通道极限流量为：K/(ts+K*td),再上下都除以K；
    - 选择通道每次选择一台设备传完N个字节，则通道极限流量为N/(TS+N*TD),再上下都除以N；
    - 字节方式，应该计算通道接的各个设备的字节传送速率之和；
    - 数组和选择方式，计算通道接的各个设备的字节传送速率中最大者；
    - 求出通道极限流量后，单位为mb/s，则根据前面通道极限流量的计算可算出ts+td；
- 程序员编程用的地址与主存物理地址的概念，关系
    - 主存物理地址是程序在主存中的实际地址，程序员编程用的地址是逻辑地址，程序装入主存后在映像表中建立好程序空间和实存空间的地址映像关系，程序执行时通过查映像表将程序（虚拟）地址变换成实主存地址再访问主存
...
- 超标量处理机，超流水线处理开发并行性的方式
    - 超标量处理机采用多指令流水线，每个△t同时流出m条指令（称为度m）；利用资源重复，设置多个执行部件寄存器堆端口（配置多套功能部件、指令译码电路和多组总线，寄存器也备有多个端口和多组总线，将可并行的相邻指令送往流水线）；
    - 超流水线处理机每个△t’仍只流出一条指令，但它的△t’只是基本机器周期△t的1/m；着重开发时间并行性，在公共的硬件上采用较短的时钟周期，深度流水来提高速度；
- 阵列处理机根据存储器的组成方式哪些构型
    - 采用分布式存储器的阵列处理机构型
    - 采用集中式共享存储器的阵列处理机构型
